...
const Bookings: React.FC<BookingsProps> = ({ bookings }) => {
  ...
  return (
    <div className="space-y-4">
      {bookings.map(b => (
        <div key={b.id} className="p-4 bg-white rounded shadow flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
          ...
          <div className="mt-2 sm:mt-0">
            <span className={`px-3 py-1 rounded-full text-white ${b.status === 'pending' ? 'bg-yellow-500' : b.status === 'approved' ? 'bg-green-600' : 'bg-red-600'}`}>
              {b.status}
            </span>
          </div>
        </div>

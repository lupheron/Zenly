...
const SubscriptionCart = () => {
    ...
    if (!subscription) {
        return (
            <div className='bg-white p-4 sm:p-6 rounded-xl shadow-xl w-full h-full min-h-60 sm:min-h-80 lg:min-h-85 flex items-center justify-center'>
                <p>{t('user.subscription.notFound')}</p>
            </div>
        );
    }
    ...
    return (
        <div className='bg-white p-4 sm:p-6 rounded-xl shadow-xl w-full h-full min-h-60 sm:min-h-80 lg:min-h-85'>
            <h1 className='text-lg sm:text-xl lg:text-2xl font-bold mb-3 sm:mb-4'>{t('user.subscription.title')}</h1>
            ...
            <div className='flex flex-col gap-2'>
                <div className='flex items-center gap-x-4'>
                    <p className='font-bold tracking-[1px] text-lg'>{t('user.subscription.accountType')} </p>
                    <p className='text-purple-500 text-lg'>{subscription.plan_name}</p>
                </div>
                <div className='flex items-center gap-x-4'>
                    <p className='font-bold tracking-[1px] text-lg'>{t('user.subscription.amountPaid')} </p>
                    <p className='text-purple-500 text-lg'>{subscription.price?.toLocaleString()} so&apos;m</p>
                </div>
                <div className='flex items-center gap-x-4'>
                    <p className='font-bold tracking-[1px] text-lg'>{t('user.subscription.paidAt')} </p>
                    <p className='text-purple-500 text-lg'>{subscription.start_date}</p>
                </div>
                <div className='flex items-center gap-x-4'>
                    <p className='font-bold tracking-[1px] text-lg'>{t('user.subscription.validUntil')}  </p>
                    <p className='text-purple-500 text-lg'>{subscription.start_date} - {subscription.end_date} <span className='text-gray-500 text-sm'>{t('user.subscription.until')}</span></p>
                </div>
                <div className='flex items-center gap-x-4'>
                    <p className='font-bold tracking-[1px] text-lg'>{t('user.subscription.paymentType')}  </p>
